...
from ..models.cube import Cube, CubeQr
from ..models.multipack import Multipack, Status
from ..models.pack import Pack, PackInReport
from ..models.pack import Status as PackStatus
from ..models.packing_table import PackingTableRecord
from ..models.pintset_record import PintsetRecord
from ..models.production_batch import ProductionBatch, ProductionBatchNumber
from ..models.report import (
    AnotherCubeReportItem,
    CubeReportItem,
    CubeReportItemExtended,
    ExtendedReportResponse,
    MPackReportItem,
    MPackReportItemExtended,
    PackReportItem,
    PackReportItemExtended,
    ReportRequest,
    ReportResponse,
    ReportWithoutMPacksResponse,
)
from ..models.system_status import Mode, State, SyncState, SystemState, SystemStatus
...
async def find_not_shipped_pack_by_qr(qr: str) -> Optional[Pack]:
    ...
    multipack_containing_that_pack = await get_multipack_by_included_pack_id(pack.id)
    if multipack_containing_that_pack.status != Status.IN_CUBE:
        return pack

    last_cube = await get_last_cube_in_queue()
    if (
        str(multipack_containing_that_pack.id)
...
async def get_multipack_by_included_pack_id(id: ObjectId) -> Optional[Multipack]:
...
async def packing_table_error(error_msg: str, wrong_cube_id: ObjectId) -> SystemState:
    current_status = await get_current_status()
...
async def get_last_batch() -> ProductionBatch:
    last_batch = await engine.find_one(
        ProductionBatch, sort=query.desc(ProductionBatch.id)
    )
    return last_batch


async def get_last_packing_table_amount() -> int:
    last_record = await engine.find_one(
        PackingTableRecord, sort=query.desc(PackingTableRecord.id)
    )
    if not last_record:
        return 0
...
async def get_last_pintset_amount() -> int:
    last_record = await engine.find_one(
        PintsetRecord, sort=query.desc(PintsetRecord.id)
    )
    if not last_record:
        return 0
...
async def get_batch_by_number_or_return_last(
    ...
    if batch_number:
        batch = await engine.find_one(
            ProductionBatch, ProductionBatch.number == batch_number
        )
        if not batch:
            raise HTTPException(404)
    else:
...
async def form_cube_from_n_multipacks(n: int) -> Cube:
    ...
    for i in range(len(multipacks_for_cube)):
        ...
        multipack_ids_with_pack_ids[
            str(multipacks_for_cube[i].id)
        ] = multipacks_for_cube[i].pack_ids
...
async def generate_packs(
    ...
    for i in range(n):
        new_pack = Pack(
            qr=f'skipped pack {current_datetime.strftime("%d.%m.%Y %H:%M")} {i}',
        ...
        msg = (
            f"Добавил пачку с QR={new_pack.qr}, "
            f"id={new_pack.id}, "
...
async def generate_multipack(
    batch_number, packs_in_multipacks, current_datetime, logger, to_process
):
    packs, email_body = await generate_packs(
        n=packs_in_multipacks,
        batch_number=batch_number,
    ...
    msg = (
        f"Добавил паллету с QR={multipack.qr}, "
        f"id={multipack.id}, "
...
async def get_100_last_packing_records() -> List[PackingTableRecord]:
    records = await engine.find(
        PackingTableRecord, sort=query.desc(PackingTableRecord.id), limit=100
    )
    return records
...
async def get_100_last_pintset_records() -> List[PintsetRecord]:
    records = await engine.find(
        PintsetRecord, sort=query.desc(PintsetRecord.id), limit=100
    )
    return records
...
async def get_multipacks_queue() -> List[Multipack]:
    multipacks = await engine.find(
        Multipack, Multipack.status != Status.IN_CUBE, sort=query.asc(Multipack.id)
    )
    return multipacks


async def count_multipacks_queue() -> int:
    multipacks_amount = await engine.count(
        Multipack, Multipack.status != Status.IN_CUBE
    )
    return multipacks_amount


async def get_first_exited_pintset_multipack() -> Multipack:
    multipack = await engine.find_one(
        Multipack, Multipack.status == Status.EXIT_PINTSET, sort=query.asc(Multipack.id)
    )
...
async def count_exited_pintset_multipacks() -> int:
    return await engine.count(Multipack, Multipack.status == Status.EXIT_PINTSET)


async def get_first_wrapping_multipack() -> Multipack:
    multipack = await engine.find_one(
        Multipack, Multipack.status == Status.WRAPPING, sort=query.asc(Multipack.id)
    )
...
async def count_multipacks_entered_pitchfork() -> int:
    return await engine.count(Multipack, Multipack.status == Status.ENTER_PITCHFORK)
...
async def count_multipacks_on_packing_table() -> int:
    return await engine.count(Multipack, Multipack.status == Status.ON_PACKING_TABLE)
...
async def get_last_cube_in_queue() -> Cube:
    last_batch = await get_last_batch()
    cube = await engine.find_one(
        Cube, Cube.batch_number == last_batch.number, sort=query.desc(Cube.id)
    )
    return cube
...
async def get_packs_report(q: ReportRequest) -> List[PackInReport]:
    ...
    return await engine.find(
        PackInReport,
        query.and_(
            PackInReport.created_at < dt_end, PackInReport.created_at >= dt_begin
        ),
    )
...
async def get_report(q: ReportRequest) -> ReportResponse:
    ...
    cubes = await engine.find(
        Cube,
        query.and_(Cube.created_at < dt_end, Cube.created_at >= dt_begin),
...
async def get_extended_report(q: ReportRequest) -> ExtendedReportResponse:
    ...
    cubes_report = [parse_obj_as(CubeReportItemExtended, cube.dict()) for cube in cubes]

    for i, cube in enumerate(cubes):
        list_of_ids = [ObjectId(i) for i in cube.multipack_ids_with_pack_ids]
        multipacks = await engine.find(Multipack, Multipack.id.in_(list_of_ids))
        mpacks_report = [
            parse_obj_as(MPackReportItemExtended, mpack.dict()) for mpack in multipacks
        ]
        ...
        for j, multipack in enumerate(multipacks):
            packs = await engine.find(Pack, Pack.id.in_(multipack.pack_ids))
            packs_report = [
                parse_obj_as(PackReportItemExtended, pack.dict()) for pack in packs
            ]
            mpacks_report[j].packs = packs_report
...
async def get_report_without_mpacks(q: ReportRequest) -> ReportWithoutMPacksResponse:
    ...
    cubes_report = [parse_obj_as(AnotherCubeReportItem, cube.dict()) for cube in cubes]

    for i, cube in enumerate(cubes):
        list_of_ids = [
            inner
            for outer in cube.multipack_ids_with_pack_ids.values()
            for inner in outer
        ]
